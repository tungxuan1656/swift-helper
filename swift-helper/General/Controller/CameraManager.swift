...
public enum CameraRecordingQuality : CGFloat {
	case lowest = 0.3
	case low = 0.5
	case normal = 0.8
	case good = 1.0
...
public enum CameraSampleBufferType {
	case video
	case audio
}
...
open class CameraManager: NSObject, UIGestureRecognizerDelegate {
    ...
    open var showErrorBlock: (_ erTitle: String, _ erMessage: String) -> Void = { (erTitle: String, erMessage: String) -> Void in
		...
		if let topController = Global.Constant.window?.topViewController() {
	...
	*/
	...
	fileprivate var assetWritingQueue: DispatchQueue?
	
	fileprivate var photoCaptureHandler: ((CaptureResult) -> Void)?
	fileprivate var recordingHandler: ((Error) -> Void)?
	fileprivate var recordingOutputURL: URL?
	
	fileprivate var onRecording = false
    ...
    open func capturePictureDataWithCompletion(_ imageCompletion: @escaping (CaptureResult) -> Void) {
        ...
        sessionQueue.async {
			let photoOutput = self._getCapturePhotoOutput()
			if let connection = photoOutput.connection(with: .video), connection.isEnabled {
				if self.cameraDevice == .front, connection.isVideoMirroringSupported, self.shouldFlipFrontCameraImage {
				...
				let settings = self.getCapturePhotoSettings()
				self.photoCaptureHandler = imageCompletion
     ...
     */
	open func startRecordingVideo(_ handler: ((_ error: Error) -> Void)?) {
		// DEBUG
        _updateIlluminationMode(flashMode)
        print("Start Recording Video")
		self._setupAssetWriter()
		self.recordingHandler = handler
     ...
     */
    open func stopRecordingVideo(_ completion: ((_ videoURL: URL?) -> Void)?) {
        // DEBUG
		...
		self.assetWriter?.finishWriting(completionHandler: { [weak self] in
			...
			guard let url = self.recordingOutputURL else { return }
			if self.writeFilesToPhoneLibrary {
				if PHPhotoLibrary.authorizationStatus() == .authorized {
					self._saveVideoToLibrary(url)
				} else {
    ...
    private var avCaptureDataOutput: AVCaptureVideoDataOutput?
	...
	*/
    ...
    @objc fileprivate func _orientationChanged() {
        ...
        // }
		currentConnection = self.videoDataOutput?.connection(with: .video)
		if let location = locationManager?.latestLocation {
    ...
    fileprivate func _setupCamera(_ completion: @escaping () -> Void) {
        ...
        sessionQueue.async {
            if let validCaptureSession = self.captureSession {
				...
				validCaptureSession.sessionPreset = .photo
    ...
    fileprivate func _setupOutputMode(_ newCameraOutputMode: CameraOutputMode, oldCameraOutputMode: CameraOutputMode?) {
		...
		// tungxuan
		if let captureSession = captureSession {
			if !captureSession.outputs.contains(self.videoDataOutput!) {
				if captureSession.canAddOutput(videoDataOutput!) {
					print("Add output video data")
	...
	*/
    ...
    fileprivate func _setupOutputs() {
		if photoOutput == nil {
			photoOutput = self._getCapturePhotoOutput()
		}
		if videoDataOutput == nil {
			videoDataOutput = self._getVideoDataOutput()
		}
	...
	fileprivate func _getVideoDataOutput() -> AVCaptureVideoDataOutput {
		if videoDataOutput == nil {
			...
			videoOutput.setSampleBufferDelegate(self, queue: DispatchQueue(label: "tungxuan.cameramanager.queue"))
			videoOutput.connection(with: .video)?.videoOrientation = .portrait
			
			if let session = self.captureSession, session.canAddOutput(videoOutput) {
	...
	fileprivate func _getCapturePhotoOutput() -> AVCapturePhotoOutput {
		if self.photoOutput == nil {
			let output = AVCapturePhotoOutput()
			if let session = self.captureSession, session.canAddOutput(output) {
	...
	// MARK: - AVAssetWriter
	fileprivate func _setupAssetWriter() {
		do {
			self.assetWritingQueue = .init(label: "com.tungxuan.cameramanager.recording")
			let url = URL(fileURLWithPath: NSTemporaryDirectory().appending("CAMERAMANAGER_RECORDING_\(UUID()).mp4"))
			// let url = self._tempFilePath()
			...
			let videoSettings = [
				...
				AVVideoWidthKey : vWidth * recordingQuality.rawValue,
				AVVideoHeightKey : vHeight * recordingQuality.rawValue
			] as [String : Any]
			
			videoInput = .init(mediaType: .video, outputSettings: videoSettings)
	...
	fileprivate func _addSampleBuffer(sample: CMSampleBuffer, type: CameraSampleBufferType) {
		guard let assetWriter = self.assetWriter else { return }
		self.assetWritingQueue?.async { [weak self] in
			if CMSampleBufferDataIsReady(sample) {
				...
				if assetWriter.status == .failed {
					...
					if let h = self?.recordingHandler {
						h(assetWriter.error!)
					}
					return
				}
				...
				if type == .video {
					...
					if self?.recordUsingAudio ?? false {
	...
	fileprivate func _releaseAssetWriter() {
	...
	/*
    fileprivate func _updateFlash(_ flashMode: AVCaptureDevice.FlashMode) {
        captureSession?.beginConfiguration()
        defer { captureSession?.commitConfiguration() }
        for captureDevice in AVCaptureDevice.videoDevices {
	...
	*/
	...
	fileprivate func getCapturePhotoSettings() -> AVCapturePhotoSettings {
		...
		let camera = self.cameraDevice == .front ? self.frontCameraDevice : self.backCameraDevice
    ...
    fileprivate func _updateCameraQualityMode(_ newCameraOutputQuality: AVCaptureSession.Preset) {
        if let validCaptureSession = captureSession {
            let sessionPreset = newCameraOutputQuality
...
extension CameraManager: AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        // 1 - get image buffer from sample buffer
		// print("Sample Buffer")
		connection.videoOrientation = self._currentPreviewVideoOrientation()
		
		
		if !self.onRecording { return }
		if output == self.videoDataOutput {
			self._addSampleBuffer(sample: sampleBuffer, type: .video)
		}
		if output == self.audioDataOutput {
			self._addSampleBuffer(sample: sampleBuffer, type: .audio)
		}
    }
...
extension CameraManager: AVCapturePhotoCaptureDelegate {
	public func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto photo: AVCapturePhoto, error: Error?) {
		if let error = error {
			if let handler = self.photoCaptureHandler {
				handler(.failure(error))
			}
		}
		if let data = photo.fileDataRepresentation() {
			if let handler = self.photoCaptureHandler {
				handler(.success(content: .imageData(data)))
			}
		}
		else {
			if let handler = self.photoCaptureHandler {
				handler(.failure(CaptureError.noImageData))
			}
		}
